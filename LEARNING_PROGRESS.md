...
更新时间：2026-07-26（完成订单创建、我的订单查询和原子取消订单业务）
...
1. 检查本轮 Git diff，把订单业务代码、初始化 SQL、测试和 README 提交，再单独提交学习记录并推送。
2. 后续重构时可把三个订单方法中重复的“根据用户名查询用户”提取为私有方法，但当前不影响业务正确性，不为重构继续扩大范围。
3. 下次订单复习不看代码复述“创建、查询本人订单、取消”的输入、数据表变化和事务作用。
4. 订单模块收尾后回到 Java/Spring 核心交替学习；不继续扩展支付、物流、购物车等超出当前实习目标的业务。
...
- 已能说明订单事务用于保证订单状态、商品库存和库存记录一起提交或回滚；原子取消 SQL 的并发防重原理已理解，但仍需完成重复取消实测和单元测试后再视为稳定掌握。
